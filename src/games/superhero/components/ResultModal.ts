...
export function renderResultModal(
  ...
  gameName: string = "Guess the Superhero",
  entityLabel: string = "character"
): string {
  const publisher = entity.metadata?.publisher ? entity.metadata.publisher : "Unknown";
  const team = entity.metadata?.team ? (Array.isArray(entity.metadata.team) ? entity.metadata.team.join(", ") : entity.metadata.team) : "Independent";
  const powers = entity.metadata?.powers ? (Array.isArray(entity.metadata.powers) ? entity.metadata.powers.join(", ") : entity.metadata.powers) : "Peak Human Condition";
  ...
  return `
    <div class="entity-modal-overlay active" id="entity-result-modal">
      <div class="entity-modal-card ${won ? "modal-won" : "modal-lost"}">
        ...
        <div class="modal-entity-details" style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; margin-bottom: 1rem; width: 100%;">
          <div class="detail-pill"><span class="pill-label">Publisher:</span> <strong>${escapeHtml(String(publisher))}</strong></div>
          <div class="detail-pill"><span class="pill-label">Team:</span> <strong>${escapeHtml(String(team))}</strong></div>
          <div class="detail-pill" style="grid-column: 1 / -1;"><span class="pill-label">Powers:</span> <strong>${escapeHtml(String(powers))}</strong></div>
        </div>

        <p class="modal-summary-text">
          ${won ? `You correctly identified <strong>${escapeHtml(entity.name)}</strong> in <strong>${attempts.length} of ${maxAttempts}</strong> attempts!` : `The mystery ${entityLabel} was <strong>${escapeHtml(entity.name)}</strong>.`}
        </p>
