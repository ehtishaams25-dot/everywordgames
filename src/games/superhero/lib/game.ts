...
import { loadSuperheroDatabase, getSuperheroPool } from "./superheroes";
import { pickRandomEntity } from "./random";
import { getEntityStats, recordEntityGame } from "./stats";
import { renderCharacterImage } from "../components/CharacterImage";
...
export async function mountSuperheroGame(root: HTMLElement, gameConfig: GameConfig) {
  ...
  try {
    const db = await loadSuperheroDatabase();
    pool = getSuperheroPool(db, difficulty);
  } catch (err) {
  ...
  const render = () => {
    ...
    root.innerHTML = `
      <div class="entity-game-container">
        ...
        <div class="entity-stage-grid">
          ...
          <div class="entity-center-col">
            ...
            <div class="entity-input-container">
              ${renderGuessForm(attempts, maxAttempts, isDone, "Type Superhero name...")}
            </div>
          </div>
        ...
        <!-- Modals -->
        ${isDone ? renderResultModal(isWon, currentEntity, attempts, maxAttempts, gameConfig.name, "superhero") : ""}
